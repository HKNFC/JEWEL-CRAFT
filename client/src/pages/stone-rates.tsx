...
export default function StoneRatesPage() {
  ...
  const openNewDialog = () => {
    setEditingId(null);
    form.reset({ minCarat: "", maxCarat: "", pricePerStone: "" });
    setDialogOpen(true);
  };

  const filteredRates = stoneRates?.filter(r => 
    r.minCarat.includes(searchQuery) || 
    r.maxCarat.includes(searchQuery) ||
    r.pricePerStone.includes(searchQuery)
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold" data-testid="text-stone-rates-title">Mıhlama Listesi</h1>
          <p className="text-muted-foreground">Taş boyutuna göre mıhlama fiyatlarını yönetin</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={openNewDialog} data-testid="button-add-stone-rate">
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            ...
            <Input
              placeholder="Karat veya fiyat ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="max-w-sm"
              data-testid="input-search-stone-rate"
            />
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-muted-foreground text-center">
                {searchQuery 
                  ? "Arama kriterlerine uygun oran yok" 
